...
class Twiterseo extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('twiter_model'); // calls the model
        ...
        if ($this->session->userdata('admin_is_logged_in') != true) {
            ...
            $data = $this->login_model->page_details('twiterseo', 'admin_management_list');
            $page_id = $data[0]->id;
    ...
    function index() {
        ...
        $data['twiter_data'] = $this->twiter_model->get_twiterseo_value();
        $this->load->view('twiterseo/index', $data);
    ...
    function update() {
        if ($this->input->post('mode') == 'update') {
            ...
            $upd_data = $this->twiter_model->update_value('twiter_seo', $this->input->post('id'), $data_to_store);

            if ($upd_data) {
                $this->session->set_userdata('success_msg', 'Twiter  seo content updated successfully');
            } else {
