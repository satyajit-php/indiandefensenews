            ...
            <div class="col-md-4 col-md-offset-4">
                <div class="login-panel panel panel-default">
                    ...
                    <div class="panel-body">
                        ...
                        <form role="form" id="login_form" method="POST" action="<?php echo site_url();?>/login_cont/update">
                            <input type="hidden" id="mode_login" name="mode_login" value="after_login">
                             <input type="hidden" id="admin_id" name="admin_id" value="<?php echo $admin_exist['admin_arr'][0]->id ?>">
                            <fieldset>
                                <div class="form-group">
                                    <label>Password</label> 
                                    <input class="form-control need" placeholder="Password" label="Password" name="pass" id="pass" type="password" autofocus <?php if($this->input->cookie('cookie_value')=='checked'){?> value="<?php echo $this->input->cookie('cookie_email');?>"<?php } ?> onkeyup="get_keyval(event)">
                                    <span id="pass_error" style="color: red;"></span>
                                </div>
                                <div class="form-group">
                                     <label>Confirm Password</label> 
                                    <input class="form-control need" placeholder="Confirm Password" label=" Confirm Password" name="cpass" id="cpass" type="password" <?php if($this->input->cookie('cookie_value')=='checked'){?> value="<?php echo $this->input->cookie('cookie_password');?>"<?php } ?> onkeyup="get_keyval(event)">
                                    <span id="cpass_error" style="color: red;"></span>
                                </div>
                               
                                <!-- Change this to a button or input when using this as a form -->
                                <a href="javascript:void(0);" class="btn btn-lg btn-success btn-block" onclick="login_form_validation();">Reset Password</a>
                            </fieldset>
                        </form>
                    </div>
                </div>
        ...
        $('.need').each(function(){
            ...
            if (elem_val.search(/\S/)==-1)
            {
                ...
                if ($('#pass').val() != $('#cpass').val() ) {
                    has_error++;
                    $('#cpass_error').html("Password does not match");
                }
                //code
            }
            else
